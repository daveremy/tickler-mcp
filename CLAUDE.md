...
- **Telegram delivery is a two-phase read/claim, never a single mark-on-read (issue #11)** — `check --notify-due` is a pure read (never mutates); the external poller sends the Telegram message itself, and ONLY on a confirmed send does it call `notify-mark-fired <id> --prev-fired-at <token>`, a compare-and-swap on the `lastFiredAt` token the read handed back (reusing `claimNagFire`'s CAS, with an added `notify`-channel guard so it can never claim a different channel's tickler). Marking fired on the read would lose a tickler forever the moment the downstream send failed — this design makes that failure mode retry instead. A non-nag `telegram:dave` tickler fires exactly once, ever, because nothing re-polls it into anyone's attention the way an agent session's own `tickler_check` does; `checkTicklers` was changed to positively match `notify = 'agent'` (not a negative `!=` on the telegram value) so a future third channel can't leak into the agent path by omission.
